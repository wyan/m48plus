...
static VOID MapROM(BYTE a, BYTE b)
{
	...
	// HP39/49G, HP49G
	if (cCurrentRomType == 'E' || cCurrentRomType == 'X' || cCurrentRomType == 'Q')
	{
...
VOID Map(BYTE a, BYTE b)					// maps 2KB pages with priority
{
	...
	_ASSERT((cCurrentRomType!='E' && cCurrentRomType!='X' && cCurrentRomType!='Q') || !Chipset.P1Cfig || Chipset.cards_status == 0xF);
	...
	else									// HP48GX / HP49G
	{
		if (Chipset.P2Cfig)					// NCE3, port2
		{
			...
			if ((cCurrentRomType=='X' || cCurrentRomType=='Q') && (Chipset.IORam[LCR]&LED))
				MapROM(a,b);				// NCE3, ROM
			else
...
VOID Npeek(BYTE *a, DWORD d, UINT s)
{
	...
	do
	{
		...
		else
		{
			...
			if ((cCurrentRomType=='X' || cCurrentRomType=='Q') && (Chipset.IORam[LCR] & LED) && M_P2 == eMap)
			{
			...
			else
			{
				if ((p=RMap[u]) != NULL)	// module mapped
				{
					...
					//debug
					static BYTE * p_old1=0;
					static BYTE * p_old2=0;
					if ((u==128) && (p!=p_old1)) {
						p_old1 = p;
					}
					if ((u==129) && (p!=p_old2)) {
	...
	do
	{
		...
		else
		{
			...
			if (cCurrentRomType!='S' && M_BS == eMap)
			{
				...
				if (cCurrentRomType=='E' || cCurrentRomType=='X' || cCurrentRomType=='Q') // HP39/40G, HP49G
				{
					RomSwitch(v+c);
				}
			}
		...
		else
		{
			...
			if (cCurrentRomType!='S' && M_BS == eMap)
			{
				...
				if (bWrite)					// write cycle?
				{
					...
					// HP39/40G, HP49G
					if (cCurrentRomType=='E' || cCurrentRomType=='X' || cCurrentRomType=='Q') RomSwitch(Chipset.Bank_FF);
				}
			}
...
VOID ReadIO(BYTE *a, DWORD d, DWORD s, BOOL bUpdate)
{
		...
		switch (d)
		{
			...
			case 0x0F:
				...
				else
				{
					...
					_ASSERT((cCurrentRomType!='E' && cCurrentRomType!='X'  && cCurrentRomType!='Q') || Chipset.cards_status == 0xF);
	...
	if (rbr_acc) ; //mksg CommReceive();				// look for new character
	return;
}
...
VOID WriteIO(BYTE *a, DWORD d, DWORD s)
{
	...
	BOOL  bAnnun = FALSE;					// flag annuncaitors changed
		...
		switch (d)
		{
			...
			case 0x09:
				...
				if (c & RST)
				{
					...
					bAnnun = TRUE;				// annunciators changed
			...
			case 0x12:
				...
				{
					Chipset.IORam[d]=c;
					//mksg CommTxBRK();				// update BRK condition
#if defined DEBUG_SERIAL
					{
						TCHAR buffer[256];
			...
			case 0x1A:
				...
				if (bCommInit && ((c^Chipset.IORam[d]) & EIRU) != 0)
				{
					...
					//mksg bCommInit = CommOpen(szSerialWire,szSerialIr);
				}
				Chipset.IORam[d]=(Chipset.IORam[d]&8)|(c&7);
#if defined DEBUG_SERIAL
			{
...
finish:
	...
	{
		Chipset.start1 = Npack(Chipset.IORam+DISP1CTL,5)&0xFFFFE;
	}
	...
	{
		Chipset.start2 = Npack(Chipset.IORam+DISP2CTL,5)&0xFFFFE;
	}
	if (tbr_acc)							// addr 0x116-0x117 changed
	{
		IOBit(TCS,TBF,TRUE);				// set transmit buffer full bit
		//mksg CommTransmit();						// transmit char
	}
	if (bAnnun)								// annunciators changed
	{
		UpdateAnnunciators();
	}
	return;
