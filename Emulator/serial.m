...
static DWORD WINAPI TransmitThread(LPVOID pParam)
{
	while (bWriting)
	{
		...
		{
			DWORD dwWritten;
			WriteFile(hComm,(LPCVOID) &tbr,1,&dwWritten,NULL);
		}
	}
	return 0;
	// mksg.de // UNREFERENCED_PARAMETER(pParam);
}
...
BOOL CommOpen(LPTSTR strWirePort,LPTSTR strIrPort)
{
	...
	if (lstrcmp(strPort, _T(NO_SERIAL)))	// port defined
	{
		...
		_ASSERT(lstrlen(strPort) + 1 < ARRAYSIZEOF(szDevice));
		if (lstrlen(strPort) + 1 >= ARRAYSIZEOF(szDevice))
			return hComm != NULL;

		wsprintf(szDevice,_T("%s:"),strPort); // device name
...
VOID CommTxBRK(VOID)
{
	if (Chipset.IORam[TCS] & BRK)			// BRK condition
	{
		...
		{
			// abort data transfer
			//mksg PurgeComm(hComm,PURGE_TXABORT | PURGE_TXCLEAR);
			//mksg SetCommBreak(hComm);			// set into BRK state
		}
	...
	else
	{
		if (hComm != NULL)					// com port open
		{
			//mksg ClearCommBreak(hComm);			// clear BRK state
		}
	}
	return;
}
...
VOID CommTransmit(VOID)
{
	...
	//mksg EnterCriticalSection(&csTxdLock);
	...
	//mksg LeaveCriticalSection(&csTxdLock);
	...
	if (bTxChar)							// character to transmit
	{
		...
		if (hComm != NULL)					// com port open
		{
			//mksg SetEvent(hEventTxd);			// write TBR byte
		}
		else
		{
			IOBit(TCS,TBZ,FALSE);			// clear transmitter busy bit
...
VOID CommReceive(VOID)
{
	...
	//mksg EnterCriticalSection(&csRecvLock);
	do
	{
		...
		{
			//mksg if (ReadFile(hComm,&cBuffer,sizeof(cBuffer),&dwBytesRead,NULL) == FALSE)
			dwBytesRead = 0L;
			//mksg else							// bytes received
			//mksg	nRp = 0;					// reset read pointer
		}
	...
	while (FALSE);
	//mksg LeaveCriticalSection(&csRecvLock);
	return;
}
*/