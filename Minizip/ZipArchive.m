...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
		...
		{
			//DEBUG NSLog([NSString stringWithFormat:@"%d entries in the zip file",globalInfo.number_entry] );
		}
	}
			...
			if( fileInfo.dosDate!=0 )
			{
				...
				if( attr )
				{
					...
					if( ![[NSFileManager defaultManager] setAttributes:attr ofItemAtPath:fullPath error:nil] )
					{
						...
						//DEBUG NSLog(@"Failed to set attributes");
					}
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite addSkipBackupAttributeToItems:(BOOL) skipBackup
{
	BOOL success = YES;
	int ret = unzGoToFirstFile( _unzFile );
	unsigned char		buffer[4096] = {0};
	...
	do{
		...
		if( fp )
		{
			...
			if( fileInfo.dosDate!=0 )
			{
                ...
                // Added possibility of do not backup for iOS Data Storage Guidelines
                if (skipBackup) {
