...
- (BOOL)start:(NSError **)error
{
	...
	{
		//DEBUG NSLog(@"Failed to start HTTP Server: %@", *error);
	}
...
- (void)netServiceDidPublish:(NSNetService *)ns
{
	...
	////DEBUG NSLog(@"Bonjour Service Published: domain(%@) type(%@) name(%@)", [ns domain], [ns type], [ns name]);
}
...
- (void)netService:(NSNetService *)ns didNotPublish:(NSDictionary *)errorDict
{
	...
	////DEBUG NSLog(@"Failed to Publish Service: domain(%@) type(%@) name(%@)", [ns domain], [ns type], [ns name]);
	////DEBUG NSLog(@"Error Dict: %@", errorDict);
}
