...
- (BOOL)acceptOnAddress:(NSString *)hostaddr port:(UInt16)port error:(NSError **)errPtr
{
	...
	if (theSocket)
	{
		...
		////DEBUG NSLog(@"theSocket4: %hu", [self localPort:theSocket]);
	}
	...
	if (theSocket6)
	{
		...
		////DEBUG NSLog(@"theSocket6: %hu", [self localPort:theSocket6]);
	}
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
		...
		////DEBUG NSLog (@"AsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
		...
		(CFStreamClientContext *)(&theContext) ))
	{
		...
		////DEBUG NSLog (@"AsyncSocket %p couldn't attach read stream to run-loop,", self);
		////DEBUG NSLog (@"Error: %@", err);
		...
		(CFStreamClientContext *)(&theContext) ))
	{
		...
		////DEBUG NSLog (@"AsyncSocket %p couldn't attach write stream to run-loop,", self);
		////DEBUG NSLog (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
		////DEBUG NSLog (@"AsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen (theWriteStream))
	{
		////DEBUG NSLog (@"AsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
...
- (void)doStreamOpen
{
	...
	if ([self areStreamsConnected] && !(theFlags & kDidCallConnectDeleg))
	{
		...
		{
			////DEBUG NSLog (@"AsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			////DEBUG NSLog (@"AsyncSocket %p received unexpected CFSocketCallBackType %d.", self, type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
	/*	default:
			//DEBUG NSLog (@"AsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %d.", self, type); */
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
	/*	default:
			//DEBUG NSLog (@"AsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %d.", self, type); */
	}
}
